...
trendyol_render_cart_js = """let appliedCouponDiscount = 0;
...
def update_file(file_path):
    ...
    if re.search(pattern, code):
        new_code = re.sub(pattern, trendyol_render_cart_js, code, count=1)
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(new_code)
        print(f"Successfully updated trendyol-style renderCart in: {file_path}")
    else:
