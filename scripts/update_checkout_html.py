...
CHECKOUT_HTML = '''    <!-- Checkout Modal (iyzico 3D Secure Ready) -->
...
for fpath in glob.glob("*.html"):
    ...
    if 'id="checkoutOverlay"' in content:
        # Replace the checkout modal
        new_content = pattern.sub(CHECKOUT_HTML, content)
        if new_content != content:
            with open(fpath, "w", encoding="utf-8") as f:
                f.write(new_content)
            print(f"Updated checkout modal in {fpath}")
        else:
            print(f"Pattern did not match directly in {fpath}, checking manual replacement")
