...
# 1. Ensure scripts order: store-service.js before custom script
if '<script src="js/store-service.js' not in html:
    html = html.replace('<script src="js/app.js', '<script src="js/store-service.js?v=20260829_1200"></script>\n    <script src="js/app.js', 1)

# 2. Add complete Coupon Management JS functions before checkLoginState()
coupon_js_complete = """
        // ==========================================
        // KUPON & İNDİRİM YÖNETİMİ ENGINE
        // ==========================================
        function renderCouponsTable() {
            ...
            if (window.StoreService && typeof window.StoreService.getCoupons === 'function') {
                coupons = window.StoreService.getCoupons();
            } else {
                try {
                    coupons = JSON.parse(localStorage.getItem('mobelmor_coupons') || '[]');
                } catch(e) {}
            }
            ...
            if (!coupons || coupons.length === 0) {
                ...
                if (window.StoreService && window.StoreService.saveCoupons) {
                    window.StoreService.saveCoupons(coupons);
                } else {
        ...
        window.openEditCouponModal = function(id) {
            ...
            if (!coupons.length) coupons = JSON.parse(localStorage.getItem('mobelmor_coupons') || '[]');
        ...
        window.handleCouponFormSubmit = function(e) {
            ...
            if (window.StoreService && window.StoreService.addOrUpdateCoupon) {
                ...
                let coupons = JSON.parse(localStorage.getItem('mobelmor_coupons') || '[]');
                const newObj = {
...
# Insert coupon_js_complete before checkLoginState()
if 'renderCouponsTable' not in html:
    html = html.replace('function checkLoginState()', coupon_js_complete + '\n        function checkLoginState()', 1)

with open(eoz_path, "w", encoding="utf-8") as f:
    f.write(html)

print("Updated eoz98.html with full coupon management JS & StoreService integration!")
