...
for fpath in glob.glob("*.html"):
    ...
    new_content = re.sub(r'js/checkout-helper\.js\?v=[^"\']+', f'js/checkout-helper.js?v={VERSION}', new_content)
    if new_content != content:
        with open(fpath, "w", encoding="utf-8") as f:
            f.write(new_content)
        print(f"Updated {fpath}")

print(f"All files updated to version {VERSION}")
